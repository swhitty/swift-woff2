...
struct WOFF2Tests {
    ...
    @Test
    func `checks VT323 glyph count`() throws {
        let woff2 = try WOFF2(contentsOf: Bundle.test.url(forResource: "VT323-Regular.woff2"))
        let cgFont = try woff2.makeCGFont()

        print("VT323 WOFF2:")
        print("  - Reconstructed TTF size: \(woff2.fontData.count) bytes")
        print("  - Number of glyphs: \(cgFont.numberOfGlyphs)")
    }
